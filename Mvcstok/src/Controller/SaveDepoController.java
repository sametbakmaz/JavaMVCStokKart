...
public class SaveDepoController implements ActionListener {
	private DepoKartModel mod;
	private MainFrame mainFrame;
	private DepoKartFrame depoKartFrame;
	private SaveDepoController saveController;
	
	public SaveDepoController(DepoKartModel mod, MainFrame mainFrame, DepoKartFrame depoKartFrame){
		this.mod=mod;
		this.mainFrame=mainFrame;
		this.depoKartFrame=depoKartFrame;
		this.depoKartFrame.btnDepoKaydet.addActionListener(this);
	}
	public void start() {
//		mainFrame.setTitle("STOK KART");
	...
	private void buttonClicked(ActionEvent e) {
		if (e.getSource().equals(DepoKartFrame.btnDepoKaydet)) {// kaydet
			getValues();

			if (mod.kaydetDepo(mod)) {
				JOptionPane.showMessageDialog(null, "Depo Kartı Başarıyla Kaydedildi.");
				temizle();
			} else {
				JOptionPane.showMessageDialog(null,
						"Depo Kartı Kaydedilemedi. Lütfen Farklı Bir Stok Kodu İle Tekrar Deneyiniz.");
				temizle();
